...
/*===============================================================
  function selectors
----------------------------------------------------------------*/
bool sc_cache_ignore_hints = false;     // ignore all hints?
bool sc_cache_use_overheard_routes = true; 
...
private:
  ...
  SCMobiCache *routecache;
  char *name;
};

///////////////////////////////////////////////////////////////////////////

class SCMobiCache : public SCRouteCache {
friend class SCCache;
friend class MobiHandler;

public:
  SCMobiCache(const SCID& MAC_id, const SCID& net_id,int psize = 30,int ssize = 34 );
  SCMobiCache();
  ~SCMobiCache();
  void noticeDeadLink(const SCID&from, const SCID& to, Time t);
  // the link from->to isn't working anymore, purge routes containing
  // it from the cache
  void noticeRouteUsed(const SCPath& route, Time t,
		       const SCID& who_from);
  // tell the cache about a route we saw being used
  void addRoute(const SCPath& route, Time t, const SCID& who_from);
  // add this route to the cache (presumably we did a route request
  // to find this route and don't want to lose it)
  bool findRoute(SCID dest, SCPath& route, int for_use = 0);
...
protected:
  SCCache *primary_cache;   /* routes that we are using, or that we have reason
			     to believe we really want to hold on to */
  SCCache *secondary_cache; /* routes we've learned via a speculative process
			     that might not pan out */

#ifdef DSR_CACHE_STATS
  void periodic_checkCache(void);
  void checkRoute(SCPath *p, int action, int prefix_len);
  void checkRoute(SCPath &p, int&, int&, double&, int&, int&, double &);
#endif
};
...
/*===============================================================
 Constructors
----------------------------------------------------------------*/
SCMobiCache::SCMobiCache(): SCRouteCache()
{
  primary_cache = new SCCache("primary", 30, this);
  secondary_cache = new SCCache("secondary", 64, this);
  //secondary_cache = new SCCache("secondary", 10000, this);
...
int
SCMobiCache::command(int argc, const char*const* argv)
{
  if(argc == 2 && strcasecmp(argv[1], "startdsr") == 0)
    { 
      if (SCID(1,::SCIP) == net_id)
	trace("Sconfig %.5f using MOBICACHE", Scheduler::instance().clock());
      // FALL-THROUGH
    }
  return SCRouteCache::command(argc, argv);
}

#ifdef DSR_CACHE_STATS

void
SCMobiCache::periodic_checkCache()
{
  int c;
  int route_count = 0;
...
void
SCMobiCache::addRoute(const SCPath& route, Time t, const SCID& who_from)
// add this route to the cache (presumably we did a route request
// to find this route and don't want to lose it)
...
#ifdef DSR_CACHE_STATS
  SCPath *p = primary_cache->addRoute(rt, prefix_len);
...
void
SCMobiCache::noticeDeadLink(const SCID&from, const SCID& to, Time)
  // the link from->to isn't working anymore, purge routes containing
  // it from the cache
{
  if(verbose_debug)
...
void
SCMobiCache::noticeRouteUsed(const SCPath& p, Time t, const SCID& who_from)
// tell the cache about a route we saw being used
{
  SCPath stub;
...
#ifdef DSR_CACHE_STATS
  SCPath *p0 = secondary_cache->addRoute(stub, prefix_len);
...
bool
SCMobiCache::findRoute(SCID dest, SCPath& route, int for_me)
// if there is a cached path from us to dest returns true and fills in
// the route accordingly. returns false otherwise
...
// already
{
  ...
  if (min_cache == 1 && for_me)
    { // promote the found route to the primary cache
      ...
      // no need to run checkRoute over the SCPath* returned from
...
/*===========================================================================
  class SCCache routines
---------------------------------------------------------------------------*/

SCCache::SCCache(char *name, int size, SCMobiCache *rtcache)
{
  this->name = name;
  this->size = size;
  cache = new SCPath[size];
  routecache = rtcache;
  victim_ptr = 0;
}
...
bool 
SCCache::searchRoute(const SCID& dest, int& i, SCPath &path, int &index)
  // look for dest in cache, starting at index, 
...
SCPath*
SCCache::addRoute(SCPath & path, int &common_prefix_len)
{
  int index, m, n;
  int victim;
...
void
SCCache::noticeDeadLink(const SCID&from, const SCID& to)
  // the link from->to isn't working anymore, purge routes containing
  // it from the cache
{  
	...
	{ // for all nodes in the path
	  if (cache[p][n] == from && cache[p][n+1] == to)
		...
		cache[p].setLength(n+1); // truncate the path here
                cache[p][n].log_stat = SCLS_UNLOGGED;
              }
...
int
SCCache::pickVictim(int exclude)
// returns the index of a suitable victim in the cache
 ...
 */
void
SCMobiCache::checkRoute(SCPath & p,
...
void
SCMobiCache::checkRoute(SCPath *p, int action, int prefix_len)
{
  int c;
  int subroute_bad_count = 0;
  int tested = 0;
