...
SCFlowTable::SCFlowTable(int size_) : DRTab(size_) {
	assert (size_ > 0);
	size = 0;
	maxSize = size_;
	table = new SCTableEntry[size_];
	counter = 0;
}

SCFlowTable::~SCFlowTable() {
	delete table;
}

SCTableEntry &SCFlowTable::operator[](int index) {
	assert(index >= 0 && index < size);
	return table[index];
}

int SCFlowTable::find(nsaddr_t source, nsaddr_t destination, u_int16_t flow) {
	register int i;
	for (i=size-1; i>=0; i--)
...
int SCFlowTable::find(nsaddr_t source, nsaddr_t destination, const SCPath &route) {
	register int i;
	for (i=size-1; i>=0; i--)
		if (table[i].sourceIP == source &&
		    table[i].destinationIP == destination &&
...
void SCFlowTable::grow() {
	assert(0);
	SCTableEntry *temp;
	if (!(temp = new SCTableEntry[maxSize*2]))
		return;
	bcopy(table, temp, sizeof(SCTableEntry)*maxSize);
	delete table;
	maxSize *= 2;
	table = temp;
}

u_int16_t SCFlowTable::generateNextFlowId(nsaddr_t destination, bool allowDefault) {
...
int SCFlowTable::createEntry(nsaddr_t source, nsaddr_t destination, 
			   u_int16_t flow) {
...
void SCFlowTable::noticeDeadLink(const SCID &from, const SCID &to) {
...
static void checkDefaultFlow(SCDRTable &DRTab, const SCTableEntry &ent) {
	u_int16_t flow;
...
void SCFlowTable::cleanup() {
	...
	while (front < back) {
		...
		bcopy(table+back, table+front, sizeof(SCTableEntry)); // swap
		back--;
...
void SCFlowTable::setNetAddr(nsaddr_t net_id) {
	net_addr = net_id;
}

bool SCFlowTable::defaultFlow(nsaddr_t source, nsaddr_t destination, 
			    u_int16_t &flow) {
	return DRTab.find(source, destination, flow);
}

SCDRTable::SCDRTable(int size_) {
	assert (size_ > 0);
	size = 0;
	maxSize = size_;
	table = new SCDRTabEnt[size_];
}
...
bool SCDRTable::find(nsaddr_t src, nsaddr_t dst, u_int16_t &flow) {
	for (int i = 0; i < size; i++)
...
void SCDRTable::grow() {
	assert(0);
	SCDRTabEnt *temp;
	if (!(temp = new SCDRTabEnt[maxSize*2]))
		return;
	bcopy(table, temp, sizeof(SCDRTabEnt)*maxSize);
	delete table;
	maxSize *= 2;
	table = temp;
}

void SCDRTable::insert(nsaddr_t src, nsaddr_t dst, u_int16_t flow) {
	assert(flow & 1);
	for (int i = 0; i < size; i++) {
		if (src == table[i].src && dst == table[i].dst) {
...
void SCDRTable::flush(nsaddr_t src, nsaddr_t dst) {
	for (int i = 0; i < size; i++)
		if (src == table[i].src && dst == table[i].dst) {
...
SCARSTable::SCARSTable(int size_) {
	size = size_;
	victim = 0;
	table = new SCARSTabEnt[size_];
	bzero(table, sizeof(SCARSTabEnt)*size_);
}

SCARSTable::~SCARSTable() {
	delete table;
}

void SCARSTable::insert(int uid, u_int16_t fid, int hopsEarly) {
	int i = victim;
	assert(hopsEarly);
...
int SCARSTable::findAndClear(int uid, u_int16_t fid) {
	int i, retval;
