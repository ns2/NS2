...
int
SCSRForwarder::command(int argc, const char*const* argv)
{
...
void
SCSRForwarder::handlePktWithoutSR(Packet *p)
{
  hdr_scdsr *srh = hdr_scdsr::access(p);
  hdr_ip *iph = hdr_ip::access(p);
  //hdr_cmn *cmh =  hdr_cmn::access(p);
  SCPath path;

  /* check route cache for source route for this packet */
  //if (routecache_->findRoute(SCID(iph->daddr()>>8,::IP),path))
  if (routecache_->findRoute(SCID(iph->daddr(),::SCIP),path,0))
    {
      srh->init();
      path.fillSR(srh);
...
void
SCSRForwarder::recv(Packet* p, Handler* callback)
{
  hdr_scdsr *srh =  (hdr_scdsr*)p->access(off_sr_);
