...
class SCRouteCache;
#include <scdsr/cache_stats.h>
#endif

class SCRouteCache : public TclObject {
...
public:
  ...
  // the returned route
  virtual bool findRoute(SCID dest, SCPath& route, int for_use) = 0;
  ...
  int pre_addRoute(const SCPath& route, SCPath &rt,
		   Time t, const SCID& who_from);

  // returns 1 if the Route should be added to the cache, 0 otherwise.
  int pre_noticeRouteUsed(const SCPath& p, SCPath& stub,
			  Time t, const SCID& who_from);
  ...
  int checkRoute_logall(SCPath *p, int action, int start);
#endif
