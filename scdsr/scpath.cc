...
SCID sc_invalid_addr(0xffffffff,::SCNONE);
SCID SC_IP_broadcast(IP_BROADCAST,::SCIP);
...
SCID::dump() const
{
  ...
  assert(type == ::SCNONE || type == ::SCMAC || type == ::SCIP);
...
SCPath::SCPath(int route_len, const SCID *route)
{
  path = new SCID[MAX_SR_LEN];
...
SCPath::SCPath(const struct scdsr_addr *addrs, int len)
...
/* make a path from the bits of an NS source route header */
SCPath::SCPath(struct hdr_scdsr *srh)
{
	path = new SCID[MAX_SR_LEN];
	if (! srh->valid()) {
		len = 0;
...
/*
 * fill the hdr_scdsr according to SCPath
 */
void
SCPath::fillSR(struct hdr_scdsr *srh)
{
  for (int i = 0 ; i < len ; i++)
    {
...
SCPath::SCPath(const SCPath& old)
{
  path = new SCID[MAX_SR_LEN];
  if (old.path != NULL)
    {
...
void
SCPath::operator=(const SCPath &rhs)
...
void 
SCPath::removeSection(int from, int to)
...
void
SCPath::copyInto(SCPath& to) const
{
...
bool
SCPath::member(const SCID& id) const
// rtn true iff id is in path
{
  return member(id, sc_invalid_addr);  
}

bool
SCPath::member(const SCID& id, const SCID& MAC_id) const
...
void 
CopyIntoPath(SCPath& to, const SCPath& from, int start, int stop)
...
/*  for (c = to.len - 1; c >= 0; c--)
    {
      ...
      if (to.path[c] == ((SCPath &)from).owner())
	{
	  to.owner() = ((SCPath &)from).owner();
	  break;
	}
    } */
}
...
void
SCPath::checkpath() const
{
  for(int c = 0; c < MAX_SR_LEN; c++)
    {     
      assert(path[c].type == SCNONE ||
             path[c].type == SCMAC ||
             path[c].type == SCIP);
    }
}
