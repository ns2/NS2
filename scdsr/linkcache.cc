...
#define LINK_TIMEOUT MAX_SIMTIME
	...
	// mirror the values kept in class SCID
	Time	   ln_t;
...
class LinkCache : public SCRouteCache {
...
public:
	...
	bool findRoute(SCID dest, SCPath& route, int for_me = 0);
...
protected:
	...
	int addLink(const SCID& from, const SCID& to,
		    int flags, double timeout = LINK_TIMEOUT, int cost = 1);
	int delLink(const SCID& from, const SCID& to);
	Link* findLink(int from, int to);
	void purgeLink(void);
	void dumpLink(void);

	void Link_to_ID(Link* l, SCID& id);

#ifdef DSR_CACHE_STATS
	void checkLink_logall(const SCID& from, const SCID& to, int action);
	void checkLink(const SCID& from, const SCID& to, int action);
...
#define INFINITY 0x7fffffff
	...
	double find_timeout(SCID a, SCID b, bool discovered);
};

///////////////////////////////////////////////////////////////////////////

double LinkCache::find_timeout(SCID a, SCID b, bool discovered) {
	double lifetime = 0.0;
	switch (lc_exppolicy) {
...
LinkCache::LinkCache() : SCRouteCache()
{
	int i = 0;
...
void
LinkCache::noticeDeadLink(const SCID& from, const SCID& to, Time)
{
...
void
LinkCache::noticeRouteUsed(const SCPath& p, Time t, const SCID& who_from)
{
  SCPath stub;
  int c;
...
void
LinkCache::addRoute(const SCPath& route, Time t, const SCID& who_from)
{
  SCPath rt;
  int c;
...
bool
LinkCache::findRoute(SCID dest, SCPath& route, int for_me = 0)
{
	u_int32_t v;
...
void
LinkCache::Link_to_ID(Link *l, SCID& id)
{
	id.addr = l->ln_dst;
	id.type = ::IP;
	id.t = l->ln_t;
...
int
LinkCache::addLink(const SCID& from, const SCID& to,
...
int
LinkCache::delLink(const SCID& from, const SCID& to)
{
	Link *l;
	int rc = 0;
 ...
 */
void
LinkCache::checkLink_logall(const SCID& from, const SCID& to, int action)
{
	Link *l = findLink(from.addr, to.addr);
...
void
LinkCache::checkLink(const SCID& from, const SCID& to, int action)
{
	Link *l = findLink(from.addr, to.addr);
	int alive = 0;
