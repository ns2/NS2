...
public:
        ...
        TclObject* create(int, const char*const*) {
                return (new CSDSRProto);
        }
} class_DSRProto;


CSDSRProto::CSDSRProto() : Agent(PT_DSR)
{
...
void
CSDSRProto::recv(Packet* p, Handler* callback)
{
  ...
  if (net_id == SCID(1,::SCIP))
    {
      ...
      routecache->addRoute(SCPath(hsr.addrs(), hsr.num_addrs()), 0.0, SCID(1,::SCIP));
    }
  ...
  if (net_id == SCID(3,::SCIP))
    {
      ...
      routecache->addRoute(SCPath(hsr.addrs(), hsr.num_addrs()), 0.0, SCID(3,::SCIP));
    }
}


int
CSDSRProto::command(int argc, const char*const* argv)
{
  if(argc == 2) 
    {
  ...
  if(argc == 3) 
	...
	routecache = (SCRouteCache*) TclObject::lookup(argv[2]);
	if(routecache == 0)
	  return TCL_ERROR;
	return TCL_OK;
      }
...
void 
CSDSRProto::noRouteForPacket(Packet *p)
{
