...
class SCARSTable {
 public:
  SCARSTable(int size_ = ARS_TABLE_SIZE);
  ~SCARSTable();
...
struct SCDRTabEnt { /* Default Route Table Entry */
  nsaddr_t src;
  nsaddr_t dst;
  u_int16_t fid;
};
...
class SCDRTable {
  public:
    SCDRTable(int size_=FLOW_TABLE_SIZE);
    ~SCDRTable();
...
struct SCTableEntry {
    ...
    SCPath 	sourceRoute ;	// SHOULD : The complete source route.
...
class SCFlowTable
{
  ...
  private :
    ...
    SCTableEntry *table ;	// Actual flow state table
