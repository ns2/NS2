...
for {set i 0} {$i < $val(nn)} {incr i} {
    	...
    	$ns_ at [$rng uniform 1.0 4.0] "$node_($i) setdest [$rng uniform 0 1000] [$rng uniform 0 900] [$rng uniform 20 70]"
    	$node_($i) set X_ [$rng uniform 100.0 1000.0]
    	$node_($i) set Y_ [$rng uniform 50.0 950.0]
    	$node_($i) set Z_ 0.0
	$node_($i) color "green"
	if { $i > 0 } {
	set service_num [expr round([$rng uniform 1 4])]
	for {set j 0} {$j < $service_num} {incr j} {
		set service_id [expr round([$rng uniform 1 8])]
		set delay [expr round([$rng uniform 1 4])]
		set cost [expr round([$rng uniform 1 3])]
		$node_($i) addService $service_id $delay $cost
		if { $service_id == 1 } {
		$node_($i) color "red";
		$ns_ at 0.00 "$node_($i) label \"(s1 $delay $cost)\"";
		break;
		}
		if { $service_id == 2 } {
		$node_($i) color "blue"
		$ns_ at 0.00 "$node_($i) label \"(s2 $delay $cost)\"";
		break;
		}
		if { $service_id == 3 } {
		$node_($i) color "purple"
		$ns_ at 0.00 "$node_($i) label \"(s3 $delay $cost)\"";
		break;
		}
...
#$node_(0) set Z_ 0
$node_(0) color "gold"
$ns_ at 0.00 "$node_(0) label \"Max Delay 9\""
#$node_(1) set X_ 340
#$node_(1) set Y_ 300
#$node_(1) set Z_ 0
#$node_(1) color "gold"
...
set v [expr "0.4 * $i"]
$ns_ at $v "$udp_(0) send 900 \"fs:1,2,3:f1,f2,f3:9\"";
}
